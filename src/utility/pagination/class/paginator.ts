...
export default class Paginator<Entity extends ObjectLiteral> {
  ...
  private alias: string = this.paginationService.pascalToUnderscore(this.entity.name);

  private limit = 100;

  private order: Order = Order.DESC;

  public constructor(private entity: ObjectType<Entity>, private paginationKeys: Extract<keyof Entity, string>[]) {}
  ...
  private encode(entity: Entity): string {
    const payload = this.paginationKeys
      .map((key) => {
        ...
        const value = this.paginationService.encodeByType(type, entity[key]);
        return `${key}:${value}`;
      })
      .join(',');

    return this.paginationService.btoa(payload);
  }

  private decode(cursor: string): CursorParam {
    const cursors: CursorParam = {};
    const columns = this.paginationService.atob(cursor).split(',');
    columns.forEach((column) => {
      const [key, raw] = column.split(':');
      const type = this.getEntityPropertyType(key);
      const value = this.paginationService.decodeByType(type, raw);
      cursors[key] = value;
    });
