...
export const generateInsertString = payload => {
  ...
  const keysToValueString = keys => obj =>
    `(${keys
      .map(key => obj?.[key] ?? 'null')
...
export const batchInsert =
  (entityManager: EntityManager) => (tableName: string) => payload => async (returningColumns: Array<string>) => {
    const { keysString, valueString } = generateInsertString(payload);
    const returningString = returningColumns?.join?.(', ') ?? '*';
    const queryString = `
                        INSERT INTO ${tableName} ${
      keysString === '()' ? 'DEFAULT VALUES' : `${keysString} VALUES ${valueString}`
    }
...
export const generateUpdateString = payload =>
...
export const generateUpsertUpdateString = (updateColumnNames: Array<string>) => payload =>
  getUniqueKeys(payload)
