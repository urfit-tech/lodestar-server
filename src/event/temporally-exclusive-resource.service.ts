...
export class TemporallyExclusiveResourceService {
  ...
  findByTarget(appId: string) {
    return (type: TemporallyExclusiveResourceType) => async (targets: Array<string>) =>
  ...
  create = async (payload: Array<{ type: string; target: string; app_id: string }>) =>
    await batchUpsert(this.entityManager)('temporally_exclusive_resource')(['type', 'target'])(['target'])(payload)(
      null,
    );
}
